...
static jl_value_t *check_ambiguous_matches(jl_typemap_t *defs, jl_typemap_entry_t *newentry, jl_typemap_intersection_visitor_fptr fptr)
{
    ...
    struct ambiguous_matches_env env = {.match = {.fptr = fptr, .type = (jl_value_t*)type, .va = va}};
    //env.match.fptr = fptr;
...
JL_DLLEXPORT void jl_method_table_insert(jl_methtable_t *mt, jl_method_t *method, jl_tupletype_t *simpletype)
{
    ...
    if (oldvalue) {
        ...
        struct invalidate_conflicting_env env = {.match = {.fptr = invalidate_backedges, .type = (jl_value_t*)type, .va = va}};
        env.invalidated = 0;
                              ...
                              size_t world, size_t *min_valid, size_t *max_valid)
{
    ...
    struct ml_matches_env env = {.match = {.fptr = ml_matches_visitor, .type = (jl_value_t*)type, .va = va}};
    env.match.ti = NULL;
    env.match.env = jl_emptysvec;
